...
class DataManager():
    ...
    def merge(self, target, source, has_header=True):
        header_line = ""
        target_file_obj = open(target,'r')
        source_file_obj = open(source,'r')
        target_lines = target_file_obj.readlines()
        source_lines = source_file_obj.readlines()
        #if has_header :
        #    header_line = target_lines[0]
        #    target_lines = target_lines[1:]
        #    source_lines = source_lines[1:] 
            
        for aline in source_lines:
            if not(aline in target_lines):
                target_lines += [aline]
                
        target_file_obj.close()
        target_file_obj = open(target ,'w')
            
        target_file_obj.writelines(target_lines)
...
if __name__ == "__main__":
    ...
    target = r"C:\temp\SDV602\SDV602-2021-main\test_data_code\data.csv"
    source = r"C:\temp\SDV602\SDV602-2021-main\test_data_code\data copy.csv"
    data_manager.merge(target, source)
